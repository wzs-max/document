...
`优势是什么？`
1. 内存元素连续存储
2. 固定大小：创建数组时，就要确定数组的大小。
3. 存储的数据类型相同：类型相同则每个元素占的空间也是一样的。
4. 索引随机访问
`劣势是什么?`
1. 大小固定，不够灵活
2. 插入删除操作效率较低（涉及数组的扩容和数组内元素的移动）
3. 可能造成内存浪费（数组有的位置没有使用，浪费内存使用，造成空间浪费）


> **注意事项：尽量在数组初始化的时候就确定其大小，数组适合读多的场景**
...
**疑问：为什么要访问前N-1个数据？**
   ...
   链表节点没有固定的索引位置,不知道第N个节点的内存地址,只能通过前一个节点的指针找到下一个节点
...
public class ArrayQueue<E> {
    ...
    public boolean enQueue(E element){
        if (tail >= elementData.length){
            ...
            //如果头指针不在数组开始位置，说明前面有空闲空间（元素出了栈，导致头指针后移后，则流出空闲空间）
            if (head != 0){
                //进行一次数据迁移
...
1. 二叉树
2. 平衡二叉树：即ALV树，平衡二叉树是一种特殊的二叉搜索树，它的左右子树的高度差不超过1，并且左右子树都是平衡二叉树。这样可以确保树的操作（如查找、插入、删除）的时间复杂度保持在O(log n)。
3. 满二叉树
4. 完全二叉树
5. 递归树
6. 红黑树：是一种自平衡的二叉搜索树，它通过为每个节点添加颜色属性（红色或黑色）并遵循特定的规则来维持平衡。与AVL树相比，红黑树的平衡条件相对宽松，但仍然能保证O(log n)的操作时间复杂度）。
红黑树遵循以下规则：
   每个节点要么是红色，要么是黑色
   根节点是黑色
   所有叶子节点（NIL节点）是黑色
...
> 红黑树与 AVL 树相比，虽然 AVL 树更加严格平衡（AVL 树中任意两个叶子节点的高度差不超过 1），
> 但红黑树的插入和删除操作通常更高效，因为它们需要的旋转次数较少。
> 这使得红黑树在需要频繁插入和删除操作的场景中表现更好。
7. B-树：B树是一种自平衡的多路搜索树，每个节点可以包含多个键值和多个子节点，保持数据有序，支持高效的查找、插入和删除操作。
8. B+树：B+树是B树的变种，所有数据都存储在叶子节点中，内部节点只存储索引信息，叶子节点通过指针连接，形成有序链表，便于范围查询。
